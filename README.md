...
## Googleカレンダー連携（毎日自動同期）について
実施・予約記録を毎日自動でGoogleカレンダーへ反映する機能です。設定方法は [`calendar-sync/README.md`](./calendar-sync/README.md) を参照してください。
...
- `calendar-sync/` … Googleカレンダー連携（毎日自動同期）用のGoogle Apps Scriptと設定手順
